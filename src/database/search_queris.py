...
def get_attendance_by_date(attendance_date):
    conn = get_connection()
    cursor = conn.cursor()
    query = """SELECT a.employee_id, e.employee_name, a.attendance_status, a.checkin_time
               FROM attendance a
    ...
    # Convert result to a list of dictionaries (optional, but useful for rendering)
    attendance_data = [
